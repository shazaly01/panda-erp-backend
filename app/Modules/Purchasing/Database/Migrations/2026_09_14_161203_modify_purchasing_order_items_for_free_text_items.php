...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        Schema::table('purchasing_order_items', function (Blueprint $table): void {
            $table->dropForeign(['product_id']);
            $table->dropForeign(['product_unit_id']);
        });

        // 2. تعديل الحقول لتصبح nullable وإضافة حقل اسم الصنف الحر
        Schema::table('purchasing_order_items', function (Blueprint $table): void {
    ...
    {
        // 1. إسقاط المفاتيح الأجنبية
        Schema::table('purchasing_order_items', function (Blueprint $table): void {
            $table->dropForeign(['product_id']);
            $table->dropForeign(['product_unit_id']);
        });

        // 2. التراجع عن التعديلات وحذف عمود اسم الصنف
        Schema::table('purchasing_order_items', function (Blueprint $table): void {
