...
class StoreProductRequest extends FormRequest
{
    ...
    /**
     * قواعد التحقق المطبقة على طلب إنشاء المنتج
     */
    public function rules(): array
    {
        return [
            ...
            'sku' => [
                ...
                'string',
                'max:100',
                Rule::unique('inventory_products', 'sku')->where(fn ($q) => $q->whereNull('deleted_at')),
            ],
            ...
            'units.*.barcodes.*' => [
                ...
                'max:100',
                'distinct',
                Rule::unique('inventory_product_barcodes', 'barcode')->where(fn ($q) => $q->whereNull('deleted_at')),
            ],
    ...
    public function messages(): array
    {
        return [
            ...
            'type.in' => 'نوع المنتج المحدد غير صالح (storable, raw_material, composite, service).',
            'inventory_policy.required' => 'سياسة خصم/إصدار المخزون مطلوبة.',
            'inventory_policy.in' => 'سياسة المخزون غير صالحة.',
            'tracking_type.required' => 'سياسة التتبع مطلوبة.',
            'tracking_type.in' => 'سياسة التتبع يجب أن تكون (none, by_batch, by_serial).',
            'valuation_method.required' => 'طريقة التقييم المالي مطلوبة.',
            'valuation_method.in' => 'طريقة التقييم يجب أن تكون (avco, fifo, standard).',
            'units.required' => 'يجب إضافة وحدة قياس واحدة على الأقل للمنتج.',
            'units.min' => 'يجب إضافة وحدة قياس واحدة على الأقل للمنتج.',
            'units.*.unit_id.required' => 'وحدة القياس مطلوبة لكل عنصر.',
            'units.*.unit_id.exists' => 'وحدة القياس المحددة غير موجودة.',
            'units.*.conversion_factor.required' => 'معامل التحويل مطلوب.',
