...
class UpdateAdjustmentRequest extends FormRequest
{
    public function authorize(): bool
    {
        ...
        return $this->user()->can('update', $adjustment);
    }

    public function rules(): array
    {
        $adjustmentId = $this->route('adjustment')?->id ?? $this->route('adjustment');

        return [
            'adjustment_number'           => ['required', 'string', 'max:100', Rule::unique('inventory_adjustments', 'adjustment_number')->ignore($adjustmentId)->whereNull('deleted_at')],
